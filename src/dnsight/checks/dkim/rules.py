...
def merge_selector_names(user: list[str]) -> list[str]:
    """User-configured selectors first, then common defaults; dedupe, preserve order."""
    seen: set[str] = set()
    out: list[str] = []
    for s in user:
        t = (s or "").strip()
        ...
        seen.add(t)
        out.append(t)
    for s in defaults.DEFAULT_DKIM_COMMON_SELECTORS:
        if s not in seen:
            seen.add(s)
            out.append(s)
    return out
...
async def collect_dkim_data(
    ...
    """Resolve TXT for each merged selector; build DKIMData (no validation issues)."""
    names = merge_selector_names(list(dkim_cfg.selectors))
    ...
    return DKIMData(selectors_tried=names, selectors_found=results)
...
def validate_dkim_results(
    ...
    any_valid_key = False
    added_stronger_alg_rec = False

    for row in data.selectors_found:
        parsed = (
            parse_dkim_txt(row.raw_record or "")
            if row.raw_record
            else ParsedDKIMRecord(
                version_ok=False,
                key_type=None,
                public_key_b64=None,
                hash_algorithms=(),
        ...
        if not row.found:
            ...
            issues.append(
                Issue(
                    ...
                    remediation="Publish a DKIM TXT record at selector._domainkey for this domain.",
                )
            )
            continue
        ...
        if not parsed.version_ok:
            ...
            issues.append(
                Issue(
                    ...
                    description=f"Selector {row.selector!r}: expected v=DKIM1.",
        ...
        if parsed.public_key_b64 is not None and parsed.public_key_b64.strip() == "":
            ...
            issues.append(
                Issue(
                    ...
                    description=f"Selector {row.selector!r}: p= is empty (key revoked).",
        ...
        if not parsed.key_type:
            ...
            issues.append(
                Issue(
                    ...
                    description=f"Selector {row.selector!r}: missing k= tag.",
        ...
        if parsed.public_key_b64 is None:
            ...
            issues.append(
                Issue(
                    ...
                    description=f"Selector {row.selector!r}: missing p= tag.",
        ...
        if bits is None and parsed.key_type.lower() == "rsa":
            ...
            issues.append(
                Issue(
                    ...
                    description=f"Selector {row.selector!r}: p= is not valid RSA DER.",
        ...
        if hit is not None:
            ...
            issues.append(
                Issue(
                    ...
                    description=(
                        f"Selector {row.selector!r}: algorithm or hash matches "
                        f"disallowed list ({hit!r})."
                    ),
                    remediation="Use a stronger key type or hash (e.g. rsa-sha256, ed25519).",
                )
            )
            if not added_stronger_alg_rec:
                added_stronger_alg_rec = True
        ...
        if bits is not None and bits < min_bits:
            ...
            issues.append(
                Issue(
                    ...
                    description=(
                        f"Selector {row.selector!r}: key is {bits} bits; "
    ...
    if not any_valid_key and data.selectors_tried:
        recommendations.append(
            Recommendation(
                id=DKIMRecommendationId.ADD_COMMON_SELECTORS,
                title="Try common DKIM selectors",
                description=(
                    "No selector had a valid DKIM key. Vendors often use names like "
                    "default, google, or selector1. Add your provider's selector to "
                    "dkim.selectors in config or publish keys at common selector names."
                ),
            )
        )
