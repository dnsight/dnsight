"""Optional ergonomic aliases over :func:`~dnsight.sdk.run.run_check` / :func:`~dnsight.sdk.generate.generate`.

For a single generic API, prefer :func:`~dnsight.sdk.run.run_check` with a name from
:func:`~dnsight.core.registry.all_checks`. These bindings fix the registry name and add
typed ``check_<name>`` (async) and ``check_<name>_sync`` callables plus ``generate_*``.

Programmatic overrides use ``config`` and optional ``config_slice`` (the matching
:class:`~dnsight.core.config.blocks.Config` field) when ``mgr`` is unset. See
:mod:`dnsight.sdk._bindings` for the factories.
"""
